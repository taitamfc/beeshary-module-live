...
$(document).ready(function() {
    ...
    if (autoLocate && typeof navigator.geolocation != 'undefined') {
        navigator.geolocation.getCurrentPosition(function (position) {
            ...
            if (idProductLoad || idStoreLoad) {
				...
				var pp_badge = $('#pp_badge').val();
                if (pp_badge) {
                    data['pp_badge'] = pp_badge;
                }
                ...
                $.ajax({
                    ...
                    success: function (result) {
                        ...
                        if (result.hasError) {
							...
							if( result.pp_theme ){
								StoreInsertParam('savoir',result.pp_theme);
							}
							if( result.pp_badge ){
								StoreInsertParam('badge',result.pp_badge);
							}
    ...
    $("#wk_store_reset").on("click", function() {
        location.href = '/module/mpstorelocator/storedetails?stores=1';
...
function setInfoWindow(marker, markerIndex, storeLocations, infoWindow, distance = -1) {
    ...
    google.maps.event.addListener(marker, 'click', (function (marker, i) {
		var btnGetDirections = 'Voir la boutique';
        var html = '';
        html += '<div class="wk-store-hour">';
        html += '<table cellpadding="0">';
        html += '<tr><td style="vertical-align: top" class="img_map_container">';
		if( typeof storeLocations.seller.profile_image != "undefined" ){
            if(storeLocations.seller.profile_image.length === 0 ){
                var profile_image_url = '/modules/marketplace/views/img/seller_img/default-artisan-pic.png';
               
            }
            else{
                var profile_image_url = '/modules/marketplace/views/img/seller_img/' + storeLocations.seller.profile_image;
            }
		...
		var profession;
		if( typeof storeLocations.custom_fields.profession != 'undefined' ){
			profession = storeLocations.custom_fields.profession;
		}
		
        html += 'class="wkstore-name name_map">' + storeLocations.seller.seller_firstname + ' </a>';
		html += '<div class="padding-2 profession">' + profession + ' </div>';
        if (controller != 'order') {
            html += '<div class="padding-2 adresse">' + storeLocations.address1 + '</div>';
			html += '<div class="padding-2 ville">';
			html += ' ' + storeLocations.zip_code;
            html += ' - ' + storeLocations.city_name;
		...
		var profile_link = '/profile/'+storeLocations.seller.link_rewrite;
...
function StoreInsertParam(key,value)
{
