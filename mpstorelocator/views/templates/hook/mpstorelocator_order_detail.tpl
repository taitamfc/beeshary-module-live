...
{if isset($products) && $products}
    <div class="box wk-box">
        <div class="h5">{l s='Store product pickup location'}</div>
                    ...
                    <tr>
                        ...
                        <td rowspan="{$orderedProduct.count}" class="wk-border-bottom">
                            <div class="wkstore-pickup-name"><b> {$stores[$orderedProduct.id_store].name} </b></div>
                                ...
                                <a class="" href="http://maps.google.com/maps?saddr=&daddr=({$stores[$orderedProduct.id_store].latitude},{$stores[$orderedProduct.id_store].longitude})" target="_blank">
                        ...
                        <td rowspan="{$orderedProduct.count}" class="wk-border-bottom">
                            <div class="table" id="wk_pickup_details">
                                ...
                                <div class="row">
                                    <span class="col-md-3"><b>{l s='Payment Method' mod='wkstorelocator'}</b></span>
                                    <span class="col-md-8">
                                        {foreach $stores[$orderedProduct.id_store].payment_options as $payment}
                                            <img src="{$imagePath}/{$payment.id_wk_store_pay}.jpg" class="img-responsive" title="{$payment.payment_name}"/>
                                        {/foreach}
                                    </span>
                                </div>
                                {/if}
                        ...
                        {/if}
                        <span class="hidden-xs-up">{$count++}</span>
                    </tr>
                {/foreach}
            {/foreach}
        </table>
