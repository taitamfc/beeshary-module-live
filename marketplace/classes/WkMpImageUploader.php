        ...
        if ($validate) {
            ...
            if (is_array($field['name'])) {
                for ($i = 0; $i < count($field['name']); ++$i) {
                    ...
                    if (!$isFileRemoved && $this->validate(array_merge($metas, array('index' => $i, 'tmp' => $tmp_name))) && $this->uploadFile($tmp_name, $metas['file'])) {
            ...
            foreach ($_POST as $key => $value) {
                ...
                if (isset($matches) && !empty($matches)) {
                    ...
                    if ($this->isJson($input)) {
                        $removedFiles = Tools::jsonDecode($input, true);
                    }
