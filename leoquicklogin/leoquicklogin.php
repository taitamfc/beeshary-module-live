...
//            $this->context->smarty->assign('errors', $this->errors);
    ...
    public function hookDisplayBeforeBodyClosingTag($params)
    {
		...
		//cart
		$cr_id_product 		= (int)Tools::getValue('id_product');
		...
		if( $cr_id_product ){
			...
			if (is_array($cart_products)) {
				foreach ($cart_products as $cart_product) {
					...
					$bookingProductInfo 	= $objBookingProductInfo->getBookingProductInfo(0, $id_product);
					$has_booking			= ($bookingProductInfo) ? 1 : 0;
				}
			}
			
			
			$bookingProductInfo 	= $objBookingProductInfo->getBookingProductInfo(0, $cr_id_product);
			$new_has_booking 		= ($bookingProductInfo) ? 1 : 0;
			if( $has_booking != $new_has_booking && count($cart_products) > 0 ){
		...
		if (Configuration::get('LEOQUICKLOGIN_ENABLE') && !$this->context->customer->isLogged()) {
            ...
            $output .= $this->fetch('module:leoquicklogin/views/templates/front/BeforeBodyClosingTag.tpl');
    ...
    public function hookDisplayAfterBodyOpeningTag($params)
    {
		...
		if (isset($this->context->customer->id)) {
			...
			if ($seller && $seller['active']) {
				$cr_is_seller 	= true;
				$cr_is_partner 	= WkMpSeller::checkIsPartner( $seller['id_seller'] );
			}
		}
		...
		if ($shopLinkRewrite) {
			...
			if ($mpSeller && $mpSeller['active']) {
				$is_seller 		= true;
				$is_partner 	= WkMpSeller::checkIsPartner( $mpSeller['id_seller'] );
			}
        }
		
		$id_product = (int)Tools::getValue('id_product');
		if ($id_product) {
            $mpSellerByProduct = Db::getInstance()->getRow( 'SELECT id_seller FROM  `'._DB_PREFIX_.'wk_mp_seller_product` WHERE `id_ps_product` = '.(int) $id_product );
			if( $mpSellerByProduct && isset( $mpSellerByProduct['id_seller'] ) ){
				$is_partner 	= WkMpSeller::checkIsPartner( $mpSellerByProduct['id_seller'] );
			}
        }
		
		$partner_color = Db::getInstance()->getRow( 'SELECT badge_color FROM  `'._DB_PREFIX_.'mp_badges` WHERE active = 1 AND badge_is_partner = 1');
		if( $partner_color ){
			$partner_color = $partner_color['badge_color'];
		}
		$this->smarty->assign(array(
            'cr_is_seller' 		=> $cr_is_seller,
            'cr_is_partner' 	=> $cr_is_partner,
            'is_seller' 		=> $is_seller,
		...
		if (Configuration::get('LEOQUICKLOGIN_SOCIALLOGIN_ENABLE') && !$this->context->customer->isLogged()) {
			...
			$output = $this->fetch('module:leoquicklogin/views/templates/front/AfterBodyOpeningTag.tpl');
            return $output;
		}
    }
    ...
    public function correctModule()
    {
        ...
        foreach ($this->hook_support as $value) {
            ...
            $sql .= 'UPDATE `'._DB_PREFIX_.'configuration` SET `name`=\'' . pSQL($upper_name) . '\' WHERE (`name`= \'' . pSQL($name) . '\' );' . "\n";
