            ...
            foreach ($f['form']['input'] as $j => $input) {
                if (isset($obj->{trim($input['name'])})) {
                    ...
                    if ($input['name'] == 'image_link' && $obj->image != '') {
                        ...
                        if (apPageHelper::getThemeName()) {
                            $thumb = _THEMES_DIR_.apPageHelper::getThemeName().'/assets/img/modules/leoblog/'.$id_shop.'/c/'.$obj->image;
                        } else {
                            $thumb = _PS_THEME_DIR_.'/assets/img/modules/leoblog/'.$id_shop.'/c/'.$obj->image;
                        }
