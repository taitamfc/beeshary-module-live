...
<div class="container">
	<section id="content">
		<form method="post" target="_parent" action="'.(($popuplogin->psversion()==5 || $popuplogin->psversion()==6 ? $link->getPageLink("authentication"):$link->getPageLink("authentication.php"))).''.(Tools::getValue('back','false')!='false' ? '?back='.Tools::getValue('back'):'').'" style="height:260px; overflow:hidden;">
			<h1>'.$popuplogin->llogin.'</h1>
			<div style="margin-top:30px;">
				<input type="text" name="email" placeholder="'.$popuplogin->lemailaddress.'" required="yes" id="username">
			</div>
			<div>
			    <input type="hidden" name="submitLogin" value="1">
				<input type="password" name="password" placeholder="'.$popuplogin->lpassword.'" required="yes" id="password">
			</div>'.$popuplogin->runhook('popuplogin').'
			<div>
                <input type="submit" id="submit2" name="SubmitLogin" value="'.$popuplogin->lletmein.'">
...
body {
	background: #DCDDDF url(./images3/7AF2Qzt.png);
	color: #000;
	font: 14px Arial;
	margin: 0 auto;
	padding: 0;
...
.container { margin: 25px auto; position: relative; width: 600px; }
#content {
	background: #f9f9f9;
...
#content form input[type=\"password\"] {
	...
	background: #eae7e7 url(./images3/8bcLQqF.png) no-repeat;
