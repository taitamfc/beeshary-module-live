...
class MondialRelay extends Module
{
    ...
    */
    ...
    public function hookDisplayOrderDetail($params)
    {
        ...
        $this->context->controller->registerJavascript('module-'.$this->name.'-common', 'modules/'.$this->name.'/views/js/order_detail.js');
    ...
    */
    public function hookHeader($params)
    {
        ...
        if (in_array($file, array('order-opc', 'order', 'orderopc', 'amzpayments'))) {
            ...
            $this->context->controller->registerJavascript($this->name.'-gmap-mr', 'https://maps.google.com/maps/api/js?key=AIzaSyBhM-UroO07icRa3U3ISAd16lf1rNoKNNs', array('server'=>'remote'));
            $this->context->controller->addJqueryPlugin('fancybox');
            $this->context->controller->registerJavascript('module-'.$this->name.'-common', 'modules/'.$this->name.'/views/js/common.js');

            if (Configuration::get('MONDIAL_RELAY_MODE') == 'widget') {
                if (isset($this->context) && method_exists($this->context->controller, 'registerJavascript')) {
                    $this->context->controller->registerJavascript('module-'.$this->name.'-jqueryshoppicker', 'modules/'.$this->name.'/views/js/jquery.plugin.mondialrelay.parcelshoppicker.2.0.0.js');
                    $this->context->controller->registerJavascript('module-'.$this->name.'-mondialrelaywidget', 'modules/'.$this->name.'/views/js/mondialrelay_widget.js');
                }
                return $this->fetchTemplate('/views/templates/front/', 'header_widget');
            } else {
                if (isset($this->context) && method_exists($this->context->controller, 'registerJavascript')) {
                    $this->context->controller->registerJavascript('module-'.$this->name.'-mondialrelay', 'modules/'.$this->name.'/views/js/mondialrelay.js');
                    $this->context->controller->registerJavascript('module-'.$this->name.'-gmaps', 'modules/'.$this->name.'/views/js/gmap.js');
                }
    ...
    public function hookFooter($params)
    {
        ...
        if (in_array($file, array('amzpayments'))) {
