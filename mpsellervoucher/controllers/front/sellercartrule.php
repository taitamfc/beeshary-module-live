                ...
                if ($mp_seller_details && $mp_seller_details['active']) {
                    ...
                    if (!Tools::getValue('name_'.$default_lang)) {
                        if (Configuration::get('WK_MP_MULTILANG_ADMIN_APPROVE')) {
                            ...
                            $this->errors[] = Tools::displayError($this->module->l('Nom du bon de réduction est requis ').$seller_lang['name']);
                        } else {
                            $this->errors[] = Tools::displayError($this->module->l('Nom du bon de réduction est requis'));
                        }
                    }
                    else {
                        foreach ($languages as $language) {
                            ...
                            if ($value && Tools::strlen($value) > $rules['sizeLang']['name']) {
                                $this->errors[] = sprintf(Tools::displayError('Nom du bon de réduction est trop long (%2$d car. max).'), $rules['sizeLang']['name']);
                            }
                            if (!Validate::isCleanHtml(Tools::getValue('name_'.$language['id_lang']))) {
                                $lang = Language::getLanguage((int)$language['id_lang']);
                                $this->errors[] = Tools::displayError($this->module->l("Le nom du bon de réduction n'est pas valide ").$lang['name']);
                            }
                        }

                        if (Tools::getValue('for_customer')) {
                            if (!Validate::isUnsignedId(Tools::getValue('for_customer'))) {
                                $this->errors[] = Tools::displayError($this->module->l('Veuillez sélectionner un client valide.'));
                            }
                        }
                        ...
                        if (!Tools::getValue('reduction_type')) {
                            $this->errors[] = Tools::displayError($this->module->l('Veuillez sélectionner le type de réduction.'));
                        }
                        else {
                            if (!Validate::isInt(Tools::getValue('reduction_type'))) {
                                $this->errors[] = Tools::displayError($this->module->l("Une erreur s'est produite. Veuillez réessayer."));
                            } else {
                        ...
                        if (!Tools::getValue('reduction_for')) {
                            $this->errors[] = Tools::displayError($this->module->l('Veuillez sélectionner une valeur dans laquelle la remise sera appliquée.'));
                        } else {
                            if (!Validate::isInt(Tools::getValue('reduction_for'))) {
                                ...
                                if (Tools::getValue('reduction_for') == 1) {
                                    if (!Tools::getValue('mp_reduction_product')) {
                                        $this->errors[] = Tools::displayError($this->module->l('Veuillez sélectionner un produit pour lequel vous créez un bon.'));
                                    } else {
                                        if (!Validate::isInt(Tools::getValue('mp_reduction_product'))) {
                                            $this->errors[] = Tools::displayError($this->module->l('Veuillez sélectionner un produit valide.'));
                                        }
                                        else {
                                            ...
                                            if ($id_seller != $mp_seller_id) {
                                                $this->errors[] = Tools::displayError($this->module->l('Veuillez sélectionner votre produit dans la liste déroulante.'));
                                            }
                                        }
                                    }
                                    ...
                                    if (Tools::getValue('reduction_type') == 2) {
                                        ...
                                        if (!Tools::getValue('multiple_reduction_product')) {
                                            $this->errors[] = Tools::displayError($this->module->l('Sélectionnez les produits sur lesquels le bon sera appliqué.'));
                                        } else {
                                            ...
                                            foreach ($reductionProducts as $mpIdProduct) {
                                                if (!Validate::isInt($mpIdProduct)) {
                                                    ...
                                                    if ($id_seller != $mp_seller_id) {
                                                        $this->errors[] = Tools::displayError($this->module->l("Une erreur s'est produite dans les produits sélectionnés, veuillez réessayer plus tard."));
                                                        break;
                                                    }
                                                }
                                            }
