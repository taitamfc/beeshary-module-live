...
class Mpshippingcart extends ObjectModel
{
    ...
    public function isAvailable($id_ps_carrier, $id_ps_cart)
    {
        $is_available = Db::getInstance(_PS_USE_SQL_SLAVE_)->getRow('SELECT * FROM `'._DB_PREFIX_.'mp_shipping_cart` WHERE `id_ps_carrier` = '.(int) $id_ps_carrier.' AND id_ps_cart = '.(int) $id_ps_cart);
