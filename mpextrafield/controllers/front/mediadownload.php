...
class MpExtraFieldMediaDownloadModuleFrontController extends ModuleFrontController
{
    public function initContent()
    {
        $id_delete = Tools::getValue('id_delete');
        $download_id = Tools::getValue('id_attachment');
        $admin_id_delete = Tools::getValue('admin_id_delete');
        if ($download_id) {
            $this->downloadContent($download_id);
        } elseif ($id_delete) {
            $this->deleteContent($id_delete);
        } elseif ($admin_id_delete) {
            $this->deleteAdminContent($admin_id_delete);
        }
    }
    ...
    /**
     * [deleteContent ->update file field value with 0].
     *
     * @param [type] $delete_id [file id]
     *
     * @return [type] [description]
     */
    public function deleteContent($delete_id)
    {
        $link = new Link();
        ...
        if ($custom_values) {
            ...
            if (Tools::getValue('profile')) {
                Tools::redirect($link->getModuleLink('marketplace', 'editprofile', array(
                    'updated' => 1,
                    )));
            } elseif (Tools::getValue('product')) {
                Tools::redirect($link->getModuleLink('marketplace', 'productupdate', array(
                    'id' => $mp_prod_id,
                    'edited_conf' => 1,
                    )));
            ...
            Tools::redirect($link->getModuleLink('marketplace', 'productlist', array()));
        }
    }

    /**
     * [deleteAdminContent ->update file field value with 0].
     *
     * @param [type] $delete_id [file id]
     *
     * @return [type] [description]
