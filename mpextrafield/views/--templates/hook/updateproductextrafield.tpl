			...
			<div class="required form-group">
				...
				{else}
					{foreach from=$languages item=language}
						...
						<input type="text" class="form-control wk_text_field_all wk_text_field_{$language.id_lang|escape:'htmlall':'UTF-8'}"
						name="{$label_name|escape:'htmlall':'UTF-8'}" id=""
						value="{if isset($smarty.post.{$label_name|escape:'htmlall':'UTF-8'})}{$smarty.post.{$label_name|escape:'htmlall':'UTF-8'}|escape:'htmlall':'UTF-8'}{elseif $extrafield.asplaceholder eq 0}{if isset($extrafield.default_value) && $extrafield.default_value|is_array}{$extrafield.default_value.{$language.id_lang}|escape:'htmlall':'UTF-8'}{else}{$extrafield.default_value|escape:'htmlall':'UTF-8'}{/if}{/if}"{if $extrafield.asplaceholder == 1}placeholder="{if isset($extrafield.default_value) && $extrafield.default_value|is_array}{$extrafield.default_value.{$language.id_lang}|escape:'htmlall':'UTF-8'}{else}{$extrafield.default_value|escape:'htmlall':'UTF-8'}{/if}"{/if} {if $current_lang.id_lang != $language.id_lang}style="display:none;"{/if} />
					{/foreach}
				{/if}
			</div>
