				...
				<div class="col-lg-6">
					{foreach from=$languages item=language}
						{assign var="textarea_name" value="{$extrafield.attribute_name}_`$language.id_lang`"}
							<textarea name="{$textarea_name}" id="" class="form-control wk_text_field_all wk_text_field_{$language.id_lang|escape:'htmlall':'UTF-8'}" {if $extrafield.asplaceholder eq 1}placeholder="{if isset($extrafield.default_value.{$language.id_lang}) && $extrafield.asplaceholder eq 1}{$extrafield.default_value.{$language.id_lang}|escape:'htmlall':'UTF-8'}{else if isset($extrafield.default_value) && $extrafield.asplaceholder eq 1}{$extrafield.default_value|escape:'htmlall':'UTF-8'}{/if}"{/if} {if $current_lang.id_lang != $language.id_lang}style="display:none;"{/if}>{if isset($smarty.post.{$textarea_name})}{$smarty.post.{$textarea_name}}{elseif !empty($extrafield.default_value.{$language.id_lang}) && $extrafield.asplaceholder eq 0}{$extrafield.default_value.{$language.id_lang}|escape:'htmlall':'UTF-8'}{/if}</textarea>
					{/foreach}
				</div>
			</div>
		{/if}
