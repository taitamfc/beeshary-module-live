...
class AjaxImport extends Ba_importer
{

    public $array_colum = array();
    public $context;

    public function submitAddDb()
    {
        ...
        if ($settingchoose !== "") {
            ...
            $update .= 'ba_step2=\''.pSQL($array_config).'\', date_up=NOW() ';
            $update .= 'WHERE id_importer_config=' . (int) $settingchoose;
            $db->query($update);
            $select = 'SELECT ba_step2 FROM ' . _DB_PREFIX_ . 'ba_importer_config ORDER BY id_importer_config DESC';
            $a = $db->getRow($select, false);
            Configuration::updateValue('CONFIG_SELECT_IMPORTER', $a['ba_step2']);
        }
        if ($settingchoose === "") {
            ...
            $settingchoose = (int) $a['id_importer_config'];
        }
        Configuration::updateValue('CONFIG_SELECT_IMPORTER', $array_config);
        $_post = (array) Tools::jsonDecode(Tools::getValue('arr'));
        $table_name = 'ba_importer_data_' . $settingchoose;
        $sql = 'SELECT * FROM ' . _DB_PREFIX_ . $table_name;
        if ($_post["import_header"] == 0) {
        ...
        unset($array[0]);
        $this->addValues($array_colum, $feature_column, $array);
    }
    public $array_result = array();
    public $shop_id;

    public function addValues(array $array_colum, array $feature_column, array $array)
    {
        $settingchoose = Tools::getValue('get_id_config');
        $outputFlie = _PS_MODULE_DIR_."ba_importer/cronjob/log_ajax_import_".$settingchoose.".txt";
        if (file_exists($outputFlie)) {
            @unlink($outputFlie);
        }
        $_post = (array) Tools::jsonDecode(Tools::getValue('arr'));
        $this->shop_id = Tools::getValue('shop_id');
        $db = Db::getInstance();
        ...
        // import số hàng
        $end = count($array);
        if (Tools::getValue("demo_mode") == 1) {
            if ($end > 21) {
                $end = 21;
            }
        }
        $start = Tools::getValue('product_start_import');
        $first_import = false;
        if ($start == 2) {
        ...
        if ($first_import == true) {
            $this->cleanProductsInFile($settingchoose);
        }
        ...
        $this->logImport($arr_log_import, 'ajax', $settingchoose);
        foreach ($array as $key => $row) {
            if ($key >= $start && $key <= $product_end) {
                ...
                $__status = (int) $this->dataStatus($_status);
                ...
                // id_category_default
                $x = new Category();
                $id_category_default = $x->getRootCategory()->id;
                    ...
                    foreach (@$array_colum["supplier"] as $value_sup) {
                        ...
                        if ($_post['sup_exist'] == '0') {
                            if (@$supplier_select == null) {
                                ...
                                $sql .= 'VALUES(\''.$id_supplier1.'\',\''.(int) $ba_id_shop .'\')';
                                $db->query($sql);

                                $id_country = Context::getContext()->country->id;
                                $sql = 'INSERT INTO ' . _DB_PREFIX_ . 'address(id_country,id_state,id_customer,';
                                $sql .= 'id_manufacturer,id_supplier,id_warehouse,alias,lastname,firstname,';
                                $sql .= 'address1,city,date_add,date_upd,active) ';
                                $sql .= 'VALUES(\''.$id_country.'\',\'0\',\'0\',';
                                $sql .= '\'0\',\''.$id_supplier1.'\',\'0\',';
                                $sql .= '\''.pSQL(trim($_supplier)).'\',\'supplier\',';
                                $sql .= '\'supplier\',\'supplier address\',\'supplier city\',';
                                $sql .= 'NOW(),';
                                $sql .= 'NOW(),\'1\')';
                                $db->query($sql);
                            }
                ...
                if (@$_manufacturer != null) {
                    ...
                    if ($_post['manu_exist'] == '0') {
                        if (@$manufacturer_select == null) {
                            ...
                            $sql .= 'VALUES(\''.$id_manu.'\',\''.(int) $ba_id_shop .'\')';
                            $db->query($sql);
                        }
                    }
                ...
                if ($_post["identify_existing_items"] == "Supplier reference") {
                    $sql = 'SELECT * FROM ' . _DB_PREFIX_ . 'product_supplier '
                        .'WHERE product_supplier_reference = "' . pSQL($_supplier_reference) . '"';
                    if ($results = Db::getInstance()->ExecuteS($sql, true, false)) {
                        foreach ($results as $p_row5) {
                            if ($_post["identify_existing_items"] == "Supplier reference") {
                                $id_row[] = $p_row5['id_product'];
                ...
                if (!empty($id_row)) {
                    if ($_post["existing_items"] == "Update") {
                        foreach ($id_row as $row_id) {
                            ...
                            $this->logImport($arr_log_import, 'ajax', $settingchoose);
                            // since 1.1.0
                            $this->insertProductsInFile($settingchoose, @$this->shop_id, $id_product);
                            ...
                            if (isset($array_colum["category_associated"]) || isset($array_colum["main_category"])) {
                                ...
                                $cc = Configuration::get('CONFIGN_CHARACTERS_CATEGORY', null, $id_group, $ba_id_shop);
                                $characters_category = $cc;
                                if (isset($array_colum["main_category"])) {
                                    ...
                                    foreach (@$array_colum["category_associated"] as $value) {
                                        ...
                                        if (isset($array_colum["main_category"])) {
                                            ...
                                            $sql = 'UPDATE ' . _DB_PREFIX_ . 'product_shop SET id_category_default '
                                                . '= \'' . (int) @$id_category_default . '\' WHERE '
                                                . 'id_shop=\''. (int) $ba_id_shop.'\' AND id_product='. (int) $row_id;
                                            $db->query($sql);
                                ...
                                foreach (@$array_colum["category_associated_id"] as $value) {
                                    ...
                                    foreach ($_category_associated as $item_2) {
                                        ...
                                        if (@$item_2 != null) {
                                            ...
                                            $data = array(
                                                'id_category' => @$_category_associated_select[0]['id_category'],
                                                'position' => $total_catid+1,
                                                "id_product" => $row_id
                                            );
                                            // xoa category_product neu co
                            ...
                            if ($_post["identify_existing_items_combi"] == 'Attributes') {
                                if (!empty($array_attribute)) {
                                    ...
                                    $re = $remove_image;
                                    $id_a = $this->setCombinationN($a_c, $row, $id_p, $at, $q, $out, $dp, $in_q, $re);
                                    $id_attr = $id_a;
                                    $id_attr_speciy_price = (int) $id_attr;
                                    $arr_log_import = 'finished' . "\n";
                                    $this->logImport($arr_log_import, 'ajax', $settingchoose);
                                } else {
                                    $combi_data = array();
                                ...
                                if (!empty($array_warehouse)) {
                                    ...
                                    $id_attr = (int) @$id_attr;
                                    $in_q = $increase_quantity;
                                    $this->addStockWarehouse($array_warehouse, $row, $id_product, $id_attr, $in_q);
                                    $arr_log_import = 'finished' . "\n";
                                    $this->logImport($arr_log_import, 'ajax', $settingchoose);
                                }
                                // Supplier
                                if (@$id_supplier != null) {
                                    ...
                                    $id_attr = (int) @$id_attr;
                                    $this->addSupplier($id_supplier, $id_product, $id_attr, $ref, $pr_te, $id_cur);
                                    $arr_log_import = 'finished' . "\n";
                                    $this->logImport($arr_log_import, 'ajax', $settingchoose);
                                }
                            } else {
                                ...
                                $in_q = $increase_quantity;
                                $re = $remove_image;
                                $id_a = $this->setCombinationBIEN($arr_com, $row, $id_p, $q, $out, $depen, $in_q, $re);
                                ...
                                if (!empty($id_attr)) {
                                    ...
                                    foreach ($id_attr as $key_id_attr => $value_id_attr) {
                                        ...
                                        if (!empty($array_warehouse)) {
                                            ...
                                            $i_q = $increase_quantity;
                                            $this->addStockWarehouse($array_warehouse, $row, $id_product, $id_at, $i_q);
                                        }
                                        // Supplier
                                        if (@$id_supplier != null) {
                                            $ref = $_supplier_reference;
                                            $p_t = $_supplier_price_te;
                                            $id_cur = $_supplier_id_currency;
                                            $id_at = (int) @$value_id_attr;
                                            $this->addSupplier($id_supplier, $id_product, $id_at, $ref, $p_t, $id_cur);
                                        }
                                    }
                                    $arr_log_import = 'finished' . "\n";
                                    $this->logImport($arr_log_import, 'ajax', $settingchoose);
                                    ...
                                    if (@$id_supplier != null) {
                                        ...
                                        $p_t = $_supplier_price_te;
                                        $id_cur = $_supplier_id_currency;
                                        $this->addSupplier($id_supplier, $id_product, 0, $ref, $p_t, $id_cur);
                                    }
                                    $combi_data = array();
                                    if ($_post["quantity"] == "increase_quantity") {
                                        $sql = "SELECT quantity FROM "._DB_PREFIX_."stock_available WHERE id_product = "
                            ...
                            if (!empty($array_specific)) {
                                ...
                                $this->addSpecificPrices($array_specific, $row, $id_product, $i_a_s_p, $ba_id_shop);
                                $arr_log_import = 'finished' . "\n";
                                $this->logImport($arr_log_import, 'ajax', $settingchoose);
                            }
                    ...
                    if ($_post["new_items"] == "Add") {
                            ...
                            if (@$array_colum["category_associated"]['0'] != null) {
                                ...
                                foreach (@$array_colum["category_associated"] as $value) {
                                    ...
                                    if (isset($array_colum["main_category"])) {
                                        ...
                                        $sql = 'UPDATE ' . _DB_PREFIX_ . 'product_shop SET id_category_default '
                                            . '= \'' . (int) @$id_category_default . '\' WHERE '
                                            . 'id_shop=\''. (int) $ba_id_shop.'\' AND id_product='. (int) $id_product;
                                        $db->query($sql);
                        ...
                        if ($_main_img != null) {
                            ...
                            $this->logImport($arr_log_import, 'ajax', $settingchoose);
                            @$_m_img2 = explode(',', $_main_img);
                            $this->addImages($_m_img2[0], $id_product, $array_colum["main_img"], 1);
                            $arr_log_import = 'finished' . "\n";
                            $this->logImport($arr_log_import, 'ajax', $settingchoose);
                        }
                        // Add Addtion Image
                        ...
                        if ($_post["identify_existing_items_combi"] == 'Attributes') {
                            if (!empty($array_attribute)) {
                                ...
                                $this->logImport($arr_log_import, 'ajax', $settingchoose);
                                $a_com = $array_combination_field;
                                $at = $array_attribute;
                                $quan = $_combination_quantity;
                                $out = $_out_of_stock;
                                $depen = $_depends_on_stock;
                                $id_attr = $this->setCombinationN($a_com, $row, $id_product, $at, $quan, $out, $depen);
                            ...
                            if (!empty($array_warehouse)) {
                                ...
                                $this->logImport($arr_log_import, 'ajax', $settingchoose);
                                $ipatt = (int) $id_attr;
                                $this->addStockWarehouse($array_warehouse, $row, $id_product, $ipatt);
                                $arr_log_import = 'finished' . "\n";
                                $this->logImport($arr_log_import, 'ajax', $settingchoose);
                            }
                            // Supplier
                            if (@$id_supplier != null) {
                                ...
                                $id_cur = $_supplier_id_currency;
                                $ipatt = (int) @$id_attr;
                                $this->addSupplier($id_supplier, $id_product, $ipatt, $reference, $price_te, $id_cur);
                                $arr_log_import = 'finished' . "\n";
                                $this->logImport($arr_log_import, 'ajax', $settingchoose);
                            }
                        } else {
                            ...
                            $out = $_out_of_stock;
                            $depen = $_depends_on_stock;
                            $id_attr = $this->setCombinationBIEN($arr_com, $row, $id_product, $quan, $out, $depen);
                            ...
                            if (!empty($id_attr)) {
                                ...
                                foreach ($id_attr as $key_id_attr => $value_id_attr) {
                                    ...
                                    if (@$id_supplier != null) {
                                        ...
                                        $id_cur = $_supplier_id_currency;
                                        $ipatt = (int) $value_id_attr;
                                        $this->addSupplier($id_supplier, $id_product, $ipatt, $ref, $pr_te, $id_cur);
                                    }
                                }
                                $arr_log_import = 'finished' . "\n";
                                $this->logImport($arr_log_import, 'ajax', $settingchoose);
                        ...
                        // ADD Specific Prices
                        if (!empty($array_specific)) {
                            $arr_log_import = 'Insert Specific Prices...';
                            $this->logImport($arr_log_import, 'ajax', $settingchoose);
                            $i_a_s_p = $id_attr_speciy_price;
                            $this->addSpecificPrices($array_specific, $row, $id_product, $i_a_s_p, $ba_id_shop);
                            $arr_log_import = 'finished' . "\n";
        ...
        if ($end == $result) {
            $status_import = 1;
            @unlink(_PS_MODULE_DIR_ . "ba_importer/stories/" . Tools::getValue('file_name'));
            // since 1.1.0
            $this->processProductsNotInFile($settingchoose, $_post["productsnotinfile"]);
        }
        $array_multi["status"] = $status_import;
        $array_multi["number_imported"] = $result - $start + 1;
        $array_multi["array_result"] = array_map("bautf8encode", $this->array_result);
        $array_multi["product_end"] = $product_end;
        
        $array_multi = Tools::jsonEncode($array_multi);
        $data_post = Tools::jsonEncode($_POST);
        Configuration::updateValue('CONFIGN_DATA_POST', $data_post);
        echo $array_multi;
        die;
    }
    public function addStockWarehouse($array_warehouse, $row, $id_product, $id_pro_attr, $increase = 0)
    {
        ...
        foreach ($array_warehouse as $id_warehouse => $array_values) {
            ...
            $employee = new Employee(Tools::getValue("employee_id"));
    ...
    public function setCombinationN($a_com, $row, $id_pro, $a_attr, $qty = 0, $ostock = 0, $deps = 0, $in_q = 0, $r = 0)
    {
        $_post = (array) Tools::jsonDecode(Tools::getValue('arr'));
        return parent::setCombination($_post, $a_com, $row, $id_pro, $a_attr, $qty, $ostock, $deps, $in_q, $r);
    }
    public function setCombinationBIEN($a_com, $row, $id_pro, $qty = 0, $ostock = 0, $deps = 0, $in_q = 0, $re = 0)
    {
        $_post = (array) Tools::jsonDecode(Tools::getValue('arr'));
        return parent::setCombinationBIE($_post, $a_com, $row, $id_pro, $qty, $ostock, $deps, $in_q, $re);
    }
        ...
        foreach ($array_combination_group as $key_combination_group => $value_value_attribute) {
            if (@$row[$value_value_attribute] != null) {
                $sql = "SELECT ". _DB_PREFIX_ . "attribute_lang.id_attribute FROM "
                        ...
                        . _DB_PREFIX_ . "attribute ON " . _DB_PREFIX_ . "attribute_lang.id_attribute = "
    ...
    public function updateImageShop($is_cover, $id_image, $id_product)
    {
        if (strpos(_PS_VERSION_, '1.6.0') === 0 || strpos(_PS_VERSION_, '1.5') === 0) {
            ...
            // tu prestashop 1.6.1+ them truong id_product trong bang image_shop
            if ($is_cover == 1) {
