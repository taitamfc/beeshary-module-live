...
class AdminBaCronJobsController extends ModuleAdminController
{
    ...
    public function postProcess()
    {
        ...
        ob_end_clean();
        die("cronjobs_prestashop");
    }

    protected function runModulesCrons()
    {
        $query = 'SELECT * FROM ' . _DB_PREFIX_ . 'cronjobs WHERE `active` = 1 AND `id_module` IS NOT NULL';
        $crons = Db::getInstance()->executeS($query, true, false);
        ...
        if (is_array($crons) && (count($crons) > 0)) {
            foreach ($crons as &$cron) {
                ...
                if ($module == false) {
                    $sql = 'DELETE FROM ' . _DB_PREFIX_ . $this->module->name . ' WHERE `id_cronjob` = \''
                            . (int) $cron['id_cronjob'] . '\'';
                    Db::getInstance()->execute($sql, false);
                    break;
                } elseif ($this->shouldBeExecuted($cron) == true) {
                    Hook::exec('actionCronJob', array(), $cron['id_module']);
                    $query = 'UPDATE ' . _DB_PREFIX_ . 'cronjobs SET `updated_at` = NOW(),'
                            . ' `active` = IF (`one_shot` = TRUE, FALSE, `active`) WHERE `id_cronjob` = \''
                            . $cron['id_cronjob'] . '\'';
    ...
    {
        $query = 'SELECT * FROM ' . _DB_PREFIX_ . 'cronjobs WHERE `active` = 1 AND `id_module` IS NULL';
        $crons = Db::getInstance()->executeS($query, true, false);
        ...
        if (is_array($crons) && (count($crons) > 0)) {
            foreach ($crons as &$cron) {
                if ($this->shouldBeExecuted($cron) == true) {
                    ...
                    $query = 'UPDATE ' . _DB_PREFIX_ . 'cronjobs SET `updated_at` = NOW(), `active` = '
                            . 'IF (`one_shot` = TRUE, FALSE, `active`) WHERE `id_cronjob` = \''
                            . $cron['id_cronjob'] . '\'';
                    Db::getInstance()->execute($query, false);
                }
            }
    ...
    protected function shouldBeExecuted($cron)
    {
        ...
        $a = date('D', strtotime('Sunday +' . $cron['day_of_week'] . ' days'));
        $day_of_week = ($cron['day_of_week'] == -1) ? date('D') : $a;
