...
<div class="panel">
	<div class="panel-heading">
		{l s='Add Seller bank Details' mod='mpmangopaypayment'}
	</div>
		...
		{if isset($mgp_client_id) && $mgp_client_id}
			{if isset($sellers_info)}
				<form id="{$table|escape:'htmlall':'UTF-8'}_form" class="defaultForm {$name_controller|escape:'htmlall':'UTF-8'} form-horizontal" action="{$current|escape:'htmlall':'UTF-8'}&{if !empty($submit_action)}{$submit_action|escape:'htmlall':'UTF-8'}{/if}&token={$token|escape:'htmlall':'UTF-8'}" method="post" enctype="multipart/form-data" {if isset($style)}style="{$style|escape:'htmlall':'UTF-8'}"{/if}>
					...
					<div class="form-group seller_bank_details" id="mgp_owner_address_country">
						<label for="mgp_owner_country" class="control-label required col-lg-3">{l s='Country :' mod='mpmangopaypayment'}</label>
						<div class="col-lg-6">
							{if isset($countries)}
								<select name="mgp_owner_country" id="mgp_owner_country" class="form-control" style="width:300px;">
			...
			{else}
				<div class="alert alert-warning">
					{l s = 'Seller not found.' mod='mpmangopaypayment'}
				</div>
			{/if}
		{else}
			<div class="alert alert-warning">
				{l s = 'Please save you mangopay configuration first.' mod='mpmangopaypayment'}
			</div>
		{/if}
	</div>
</div>
