        ...
        if (Validate::isLoadedObject($this->context->cart) && $this->context->cart->OrderExists() == false) {
            ...
            if (Module::isInstalled('marketplace') && Module::isEnabled('marketplace') && Module::isInstalled('mpcartordersplit') && Module::isEnabled('mpcartordersplit')) {
                    ...
                    foreach ($packages as $id_package => $package) {
                        foreach ($package['product_list'] as $product) {
                            if (Configuration::get('MP_ENABLE_CART_SPLIT')) {
                                $id_carrier = $obj_cp_map->getIdCarrierByProduct($product['id_product'], $product['id_product_attribute'], $this->context->cart->id);
                                $id_carrier = $id_carrier ? $id_carrier : 0;
                            } else {
                                $id_carrier = 0;
                            }
                            ...
                            $packageKey = $id_carrier.'-'.$id_seller;
                            $orderPackage[$id_address][$packageKey]['product_list'][] = $product;
                            if (!isset($orderPackage[$id_address][$packageKey]['carrier_list'])) {
                                $orderPackage[$id_address][$packageKey]['carrier_list'] = $product['carrier_list'];
                            } else {
                                $orderPackage[$id_address][$packageKey]['carrier_list'] = array_intersect($orderPackage[$id_address][$packageKey]['carrier_list'], $product['carrier_list']);
                            }
                            if (!isset($orderPackage[$id_address][$packageKey]['warehouse_list'])) {
                                $orderPackage[$id_address][$packageKey]['warehouse_list'] = $package['warehouse_list'];
                            }
                            if (!isset($orderPackage[$id_address][$packageKey]['id_warehouse'])) {
                                $orderPackage[$id_address][$packageKey]['id_warehouse'] = $package['id_warehouse'];
                            }
                            if (!isset($orderPackage[$id_address][$packageKey]['id_carrier'])) {
                                $orderPackage[$id_address][$packageKey]['id_carrier'] = $id_carrier ? $id_carrier : $package['id_carrier'];
                            }
                        }
                    }
                }
                ...
                foreach ($orderPackage as $id_address => $packageByAddress) {
                    foreach ($packageByAddress as $id_package => $package) {
                        $package_list[$id_address][] = $package;
                    }
                }
                unset($orderPackage);
            }
            ...
            foreach ($order_detail_list as $key => $order_detail) {
                ...
                if (isset($order->id)) {
                    ...
                    $carrier = new Carrier((int)$order->id_carrier, (int)$this->context->cart->id_lang);
