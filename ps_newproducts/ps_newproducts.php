    ...
    protected function getNewProducts()
    {
        ...
        if (is_array($newProducts)) {
            foreach ($newProducts as $rawProduct) {
                if ($this->context->controller->php_self == 'product' && (int)$rawProduct['id_product'] == (int)Tools::getValue('id_product')) {
                    continue;
                }
                if (!($id_seller = Db::getInstance()->getValue('SELECT id_seller FROM `'._DB_PREFIX_.'wk_mp_seller_product` WHERE id_ps_product = '. (int)$rawProduct['id_product']))) {
                    continue;
                }

                $sellerPro = Db::getInstance()->getRow('SELECT profile_image FROM `'._DB_PREFIX_.'wk_mp_seller` WHERE id_seller = '. (int)$id_seller);
                $rawProduct['display_seller_profile_img'] = true;
                $rawProduct['seller_profile_img'] = WkMpSeller::getSellerImageLink($sellerPro);
