...
class MpShippingProduct extends ObjectModel
{
    ...
    public function findWeightInfoByMpProID($mpProductId)
    {
        $mpProductWetDetail = Db::getInstance(_PS_USE_SQL_SLAVE_)->getRow('SELECT * FROM `'._DB_PREFIX_.'mp_shipping_product` WHERE `mp_product_id` = '.(int) $mpProductId);

        if (empty($mpProductWetDetail)) {
            return false;
        } else {
            return $mpProductWetDetail;
        }
    }
