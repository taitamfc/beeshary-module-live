                    ...
                    if ($id_order_state != Configuration::get('PS_OS_ERROR') && $id_order_state != Configuration::get('PS_OS_CANCELED') && $this->context->customer->id) {
                        ...
                        if (Module::isEnabled('mpsellerinvoice')) {
                            ...
                            foreach ($orderDetails as $key => $detail) {
                                $pdf = $create_invoice->createInvoice($order, $detail, false, false, $order_status->invoice);
                                $file_attachement[$detail] = $pdf;
                            }
                            if ($order_status->pdf_invoice
                                ...
                                if (Configuration::get('MP_SELLER_INVOICE_ACTIVE') == 0) {
                                    foreach ($orderDetails as $key => $detail) {
                                        $pdf = $create_invoice->createInvoice($order, 'admin', false, false, $order_status->invoice);
                                        $file_attachement[$detail] = $pdf;
                                    }
                                }
                            }
