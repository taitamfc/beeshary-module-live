...
class CreateInvoice
{
    ...
    public function createInvoice($order, $id_seller_customer, $download = false, $admin = false, $isCreateInvoice = 0)
    {
        $orientation = 'P';
        ...
        $obj_seller = new SellerInvoice($order, $id_seller_customer, $admin);
        $this->pdf_renderer->setFontForLang(Context::getContext()->language->iso_code);
        $this->pdf_renderer->createHeader($obj_seller->getHeader($admin, $isCreateInvoice));
        $this->pdf_renderer->createFooter($obj_seller->getFooter());
        $this->pdf_renderer->createContent($obj_seller->getContent());
        ...
        if ($render) {
            ...
            $file_attachement['content'] = $this->pdf_renderer->render($obj_seller->getFilename(), $download);
            $file_attachement['name'] = $obj_seller->getFilename();
    ...
    public function orderDetailsInformation($order, $idOrderState, $download = false)
    {
        if (!$download && $idOrderState != Configuration::get('PS_OS_ERROR') && $idOrderState != Configuration::get('PS_OS_CANCELED')) {
            ...
            foreach ($order->getProducts() as $key => $product) {
                ...
                $product_price = Product::getTaxCalculationMethod() == PS_TAX_EXC ? Tools::ps_round($price, 2) : $pricewt;
                $productVarTpl = array(
                    ...
                    foreach ($customized_datas[$product['product_id']][$product['product_attribute_id']][$order->id_address_delivery] as $customization) {
                        ...
                        if (isset($customization['datas'][Product::CUSTOMIZE_TEXTFIELD])) {
                            foreach ($customization['datas'][Product::CUSTOMIZE_TEXTFIELD] as $text) {
                                $customization_text .= $text['product_name'].': '.$text['value'].'<br />';
                            }
                        }
            ...
            $productListTemp =  $this->productListTemp($productVarTplList);
            $cartRulesList = array();
            ...
            foreach ($cartRules as $cartRule) {
                ...
                $values = array(
                    'tax_incl' => $cartRule['obj']->getContextualValue(
                        true,
                        $this->context,
                        CartRule::FILTER_ACTION_ALL_NOCAP,
                        $package
                    ),
                    'tax_excl' => $cartRule['obj']->getContextualValue(
                        false,
                        $this->context,
                        CartRule::FILTER_ACTION_ALL_NOCAP,
                        $package
                ...
                if (count($orderList) == 1 && $values['tax_incl'] > ($order->total_products_wt - $total_reduction_value_ti) && $cartRule['obj']->partial_use == 1 && $cartRule['obj']->reduction_amount > 0) {
                    ...
                    $voucher->add();// If the voucher has conditions, they are now copied to the new voucher
            ...
            $cartRuleData = $this->cartRuleList($cartRulesList);
            ...
            $data = array(
                ...
                '{products}' => $productListTemp['product_list_html'],
                '{products_txt}' => $productListTemp['product_list_txt'],
                '{discounts}' => $cartRuleData['cart_rules_list_html'],
                '{discounts_txt}' => $cartRuleData['cart_rules_list_txt'],
    ...
    public function productListTemp($productVarTplList)
    {
        $productListTemp = array('product_list_txt' => '', 'product_list_html' => '');
        if (count($productVarTplList) > 0) {
            $productListTemp['product_list_txt'] = $this->getEmailTemplateContent(
                'order_conf_product_list.txt',
                Mail::TYPE_TEXT,
                $productVarTplList
            );
            $productListTemp['product_list_html'] = $this->getEmailTemplateContent(
    ...
    public function cartRuleList($cartRulesList)
    {
        $cartList = array('cart_rules_list_txt' => '', 'cart_rules_list_html' => '');
        if (count($cartRulesList) > 0) {
            $cartList['cart_rules_list_txt'] = $this->getEmailTemplateContent('order_conf_cart_rules.txt', Mail::TYPE_TEXT, $cartRulesList);
            $cartList['cart_rules_list_html'] = $this->getEmailTemplateContent('order_conf_cart_rules.tpl', Mail::TYPE_HTML, $cartRulesList);
        }
        return $cartList;
    }

    public function sendPaymentEmailToCustomer($order, $file_attachement, $admin = false, $orderState = array())
    {
        $template_path = _PS_MODULE_DIR_.'mpsellerinvoice/mails/';
        $template = 'seller_payment';
        //If we are not sending file attachment then use ps default template in mail
        if(!$file_attachement) {
            $template_path = _PS_MAIL_DIR_;
            if (isset($orderState->template) && isset($orderState->template[$order->id_lang])) {
                $template =$orderState->template[$order->id_lang];
            }
        }
        ...
        $subject = Mail::l('Payment Accepted');
        if (isset($orderState->name) && isset($orderState->name[$order->id_lang])) {
            $subject =$orderState->name[$order->id_lang];
        }
        $data['{total_paid}'] = Tools::displayPrice((float) $order->total_paid, new Currency((int) $order->id_currency), false);
        if ($admin) {
            if ($id_employee = WkMpHelper::getSupperAdmin()) {
                $obj_emp = new Employee($id_employee);
                $mail = $obj_emp->email;
                $name = $obj_emp->firstname.' '.$obj_emp->lastname;
                $data = array(
                    '{lastname}' => $obj_emp->lastname,
                    '{firstname}' => $obj_emp->firstname,
                );
            }
            $template = 'seller_to_admin';
            $subject = Mail::l('Seller Invoice');
            $template_path = _PS_MODULE_DIR_.'mpsellerinvoice/mails/';
        } else {
            $mail = $customer->email;
        }
